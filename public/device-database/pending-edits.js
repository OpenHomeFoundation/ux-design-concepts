/* AUTO-GENERATED build artifact — compiled from pending-edits.jsx.
   Do not edit this file by hand. Edit pending-edits.jsx and rebuild. */
...
function usePending(deviceId) {
  ...
  React.useEffect(() => {
    ...
    return () => {
      _pendingListeners.delete(force);
    };
  }, []);
...
function contributorFieldKeys(device) {
  ...
  return ['name', 'summary', 'photos', 'description', 'instructions', ...cat.map(f => 'spec:' + f.key), 'ecosystems', 'connectivity', 'protocols', 'bridge', 'connectsWith', 'dimensions', 'customFields', 'identifiers', 'references', 'msrp'];
}
function getField(obj, key) {
  ...
  switch (key) {
    ...
    case 'ecosystems':
      return obj.specs && obj.specs.ecosystems;
    case 'connectsWith':
      return obj.specs && obj.specs.connectsWith;
    case 'connectivity':
      return obj.specs && obj.specs.connectivity;
    case 'protocols':
      return obj.specs && obj.specs.protocols;
    case 'bridge':
      return obj.specs && obj.specs.bridge;
    case 'dimensions':
      return obj.specs && obj.specs.dimensions;
    case 'customFields':
      return obj.specs && obj.specs.customFields;
    case 'identifiers':
      return obj.specs && obj.specs.identifiers;
    case 'references':
      return obj.specs && obj.specs.references;
    default:
      return obj[key];
  }
}
...
function cleanBridge(v) {
  ...
  const arr = (Array.isArray(p) ? p : p ? [p] : []).map(x => (x == null ? '' : String(x)).trim()).filter(Boolean);
  const next = { ...v };
...
function diffContributorFields(device, draft) {
  ...
  contributorFieldKeys(device).forEach(key => {
    ...
    if (key === 'customFields') {
      a = cleanCustomFields(a);
      b = cleanCustomFields(b);
    }
    if (key === 'bridge') {
      a = cleanBridge(a);
      b = cleanBridge(b);
